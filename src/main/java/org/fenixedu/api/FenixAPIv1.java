...
import org.fenixedu.api.beans.*;
import org.fenixedu.api.beans.FenixCalendar.FenixCalendarEvent;
import org.fenixedu.api.beans.FenixCalendar.FenixClassEvent;
import org.fenixedu.api.beans.FenixCalendar.FenixEvaluationEvent;
import org.fenixedu.api.beans.FenixPayment.PaymentEvent;
import org.fenixedu.api.beans.FenixPayment.PendingEvent;
import org.fenixedu.api.beans.FenixPerson.FenixPhoto;
import org.fenixedu.api.beans.FenixPerson.FenixRole;
import org.fenixedu.api.beans.FenixPersonCourses.FenixEnrolment;
import org.fenixedu.api.beans.publico.*;
...
public class FenixAPIv1 {
    ...
    public FenixPersonCourses personCourses(@QueryParam("academicTerm") String academicTerm) {
        ...
        List<FenixEnrolment> enrolments = new ArrayList<>();
        List<FenixCourse> teachingCourses = new ArrayList<>();
    ...
    public Set<FenixDegree> degreesAll() {
        ...
        Set<FenixDegree> degrees = new HashSet<>();
    ...
    private FenixDegreeExtended getFenixDegree(ExecutionDegree executionDegree) {
        ...
        if (degreeInfo != null) {
            ...
            fenixDegreeInfo =
                    new FenixDegreeExtended.FenixDegreeInfo(description, objectives, designFor, requisites, profissionalExits, history,
                            operationRegime, gratuity, links);
        }

        final List<FenixDegreeExtended.FenixTeacher> teachers = new ArrayList<>();
        ...
        for (Teacher teacher : responsibleCoordinatorsTeachers) {
            ...
            teachers.add(new FenixDegreeExtended.FenixTeacher(teacherName, istId, mails, urls));
        }
    ...
    public List<FenixCurricularGroup> curricularGroupsByDegreeId(@PathParam("id") String oid,
        ...
        Set<FenixCurricularGroup> result = new HashSet<>();
    ...
    public FenixCourseExtended coursesByOid(@PathParam("id") String oid) {
        ...
        for (Map.Entry<CompetenceCourse, Set<CurricularCourse>> entry : curricularCourses.entrySet()) {
            ...
            for (BibliographicReference bibliographicReference : competenceCourse.getBibliographicReferences()
                ...
                biblios.add(new FenixCourseExtended.FenixCompetence.BiblioRef(author, reference, title, bibYear, type, url));
            }
            ...
            moreInfo.add(new FenixCourseExtended.FenixCompetence(competenceCourse.getExternalId(), program, biblios, degrees));

        }

        int numberOfStudents = executionCourse.getAttendsSet().size();

        List<FenixDegreeExtended.FenixTeacher> teachers = new ArrayList<>();
        for (Professorship professorship : executionCourse.getProfessorshipsSet()) {
            ...
            teachers.add(new FenixDegreeExtended.FenixTeacher(tname, istid, mail, url));
        }
